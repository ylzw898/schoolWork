...
<template>
    <div class="filter-more">
      ...
      <pay2 :list="selectrents"></pay2>
    </div>
</template>
<script>
import {getHousePrice} from '../../api/api'
...
export default {
      ...
      filterBox: [
        ...
        },
        {
          ...
          items: [
            ...
            { value: 21, text: '160~700', active: false }
          ]
        },
        {
          name: '价格(百万):',
          items: [
            ...
            { value: 23, text: '100-300', active: false },
            { value: 24, text: '300-500', active: false },
            { value: 25, text: '500-600', active: false },
            { value: 26, text: '600-800', active: false },
            { value: 27, text: '800-1000', active: false },
            { value: 28, text: '1000-5600', active: false }
          ]
        },
        {
          name: '户型:',
          items: [
            ...
            { value: 34, text: '5室', active: false },
            { value: 35, text: '6室及以上', active: false }
          ]
        }
      ]
    }
  ...
  computed: {
    // 目前没用，但可以看看
    ChooseList () {
      return this.HousePrice.filter((item) => {
    ...
    listregion () {
      ...
      if (this.selectBox2.includes(1)) {
        ...
        return this.HousePrice.filter(function (data) {
    ...
    listsize () {
      ...
      if (this.selectBox2.includes(15)) {
        ...
        return this.selectrents.filter(function (data) {
          return data.size < 700 && data.size >= 160
        })
      } else {
        return this.selectrents
      }
    ...
    listprice () {
      ...
      if (this.selectBox2.includes(22)) {
        ...
        return this.selectrents.filter(function (data) {
          return data.price < 500 && data.price >= 300
        })
      } else if (this.selectBox2.includes(25)) {
        return this.selectrents.filter(function (data) {
          return data.price < 600 && data.price >= 500
        })
      } else if (this.selectBox2.includes(26)) {
        return this.selectrents.filter(function (data) {
          return data.price < 800 && data.price >= 600
        })
      } else if (this.selectBox2.includes(27)) {
        return this.selectrents.filter(function (data) {
          return data.price < 1000 && data.price >= 800
        })
      } else if (this.selectBox2.includes(28)) {
        return this.selectrents.filter(function (data) {
          return data.price < 5600 && data.price >= 1000
        })
      } else {
        return this.selectrents
      }
